...
export default function HelpPage({ help, onAddHelp, onRespondHelp, onCompleteHelp }) {
  ...
  return (
    <div ref={scope} className="space-y-5">
      ...
      {filtered.length ? <div className="grid gap-5 lg:grid-cols-2">{filtered.map((item) => <HelpCard key={item.id} help={item} onRespond={onRespondHelp} onComplete={onCompleteHelp} />)}</div> : <EmptyState title="暂无互助需求" />}
