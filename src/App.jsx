...
const seedNotifications = [
  ...
  { id: "notice_2", title: "有新的临时求助", desc: "教学楼附近有同学需要打印帮助。", time: "12 分钟前", route: "emergency", read: false },
  { id: "notice_3", title: "公益活动报名提醒", desc: "图书馆整理活动本周六开始签到。", time: "30 分钟前", route: "volunteer", read: false }
];
...
export default function App() {
  ...
  function openDetail(module, item) {
    const maps = {
      item: {
        ...
        fields: { 分类: item.category, 方式: item.type, 位置: item.area, 新旧程度: item.condition, 联系人: item.owner, 诚信分要求: `${item.creditRequired}+` }
      },
      material: {
        module: "学习资料",
        title: item.title,
        description: item.description,
        fields: { 课程分类: item.category, 资料类型: item.type, 上传者: item.uploader, 申请次数: `${item.count} 次` }
      },
      help: {
        module: "生活互助",
        title: item.title,
        status: item.status,
        fields: { 所需物资: item.material, 地点: item.place, 期望时间: item.expectedTime, 发布者: item.publisher }
      },
      emergency: {
        module: "临时求助",
        title: item.title,
  ...
  function handleAddItem(data, error) {
    ...
    notify("闲置物品发布成功");
    pushNotification("新的闲置物品", `${data.name} 已发布，可在物品共享中查看。`, "items");
    return true;
  }
  ...
  function handleAddMaterial(data, error) {
    ...
    notify("学习资料发布成功");
    pushNotification("新的学习资料", `${data.title} 已发布到资料共享。`, "materials");
    return true;
  }
  ...
  function handleAddHelp(data, error) {
    ...
    notify("生活物资求助发布成功");
    pushNotification("新的生活互助", `${data.title} 正在等待响应。`, "help");
    return true;
  }
  ...
  function handleJoinVolunteer(id) {
    ...
    if (!applications.some((item) => item.type === "公益报名" && item.title === target?.name)) {
      ...
      pushNotification("公益报名成功", `${target?.name || "公益活动"} 已加入你的申请。`, "profile");
    }
    notify("报名成功，已加入我的申请");
  }
  ...
  function handleAddVolunteer(data, error) {
    ...
    notify("公益活动发布成功");
    pushNotification("新的公益活动", `${data.name} 已发布，可前往公益招募查看。`, "volunteer");
    return true;
  }
  ...
  const pages = {
    ...
    volunteer: <VolunteerPage volunteer={filteredState.volunteer} joined={applications.filter((item) => item.type === "公益报名").map((item) => volunteer.find((v) => v.name === item.title)?.id || item.title)} onJoin={handleJoinVolunteer} onAddVolunteer={handleAddVolunteer} onOpenDetail={(item) => openDetail("volunteer", item)} />,
