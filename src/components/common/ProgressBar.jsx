...
export default function ProgressBar({ value, className = "" }) {
  ...
  return (
    <div ref={scope} className={`h-2 overflow-hidden rounded-full bg-slate-100 ${className}`}>
      <div className="progress-fill h-full w-full origin-left rounded-full bg-campus-green" />
    </div>
  );
}
