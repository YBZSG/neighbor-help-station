...
const initial = { title: "", category: "高数", type: "笔记", uploader: "", description: "", fileName: "" };
  ...
  return (
    <form onSubmit={submit} className="grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Select label="课程分类" value={form.category} onChange={(e) => update("category", e.target.value)} options={materialCategories.filter((i) => i !== "全部")} />
        ...
        <Input label="模拟上传文件" value={form.fileName} onChange={(e) => update("fileName", e.target.value)} placeholder="例：复习提纲.pdf" />
      </div>
